...
interface PutImageFromUrlOptions
...
export type PutFromUrlCommandOptions = PutImageFromUrlOptions;

/** @deprecated Use `putImage` and `PutImageBlobResult` instead. */
export type PutFromUrlBlobResult = PutBlobResult;

/**
 * Shared implementation for `putImage` URL sources and the deprecated
 * `putFromUrl`. Not part of the public API.
 */
export async function putImageFromUrl(
  pathname: string,
  url: string,
  options: PutImageFromUrlOptions,
...
export async function putFromUrl(
  ...
  return putImageFromUrl(pathname, url, options);
}
