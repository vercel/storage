...
import { fetchWithHttpCache, cache } from './fetch-with-http-cache';
...
describe('fetchWithHttpCache', () => {
  ...
  it('should cache responses and return them from cache', async () => {
    ...
    // First request
    const data1 = await fetchWithHttpCache('https://example.com/api/data');
    ...
    const data2 = await fetchWithHttpCache('https://example.com/api/data');
  ...
  it('should differentiate caches by authorization header', async () => {
    ...
    const data3 = await fetchWithHttpCache('https://example.com/api/data', {
      headers: new Headers({ authorization: 'bearer A' }),
    });
